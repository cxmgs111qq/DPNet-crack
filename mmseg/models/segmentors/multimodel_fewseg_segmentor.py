...
class MultimodelFewSeg(BaseSegmentor):
    ...
    def __init__(self,
        ...
        # 这个模型有2个encode，一个image，一个text。一个decodehead
        # 三个部分都要加载checkpoint，包括decodehead。而传统的encode-decode结构只有backbone加载
    ...
    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        ...
        """
        classifier_embeds = self.text_encoder()#(clsnum+1)*512
        clip_inputs = inputs# b*3*640*640
        if self.asymetric_input:#非对称输入
            clip_inputs = F.interpolate(
                inputs, scale_factor=self.encoder_resolution, mode='bilinear')#2*3*320*320
        x = self.image_encoder(clip_inputs)#常规意义的编码器

        losses = dict()

        loss_decode = self._decode_head_forward_train(
            [inputs, x, classifier_embeds], data_samples) #[image(2*3*640*640)  vitout(4个stage的输出，每个stage包含（2*768*20*20)和（2*768） textout(172*512)]
    ...
    def predict(self,
        ...
        '''predict batch=1，增加支持集'''
        if data_samples[0].get('support_seg_data', None) is not None:
            # self.show(data_samples[0].support_seg_data.data[0].cpu().numpy())
            batch_img_metas[0]['support_gt']=data_samples[0].support_seg_data.data
            del data_samples[0].support_seg_data
            # inputs=inputs[:-1] #测试周期加入sup，但是网络不加入
        #
    ...
    def _forward(self,
        ...
        """
        classifier_embeds = self.text_encoder()
        # classifier_embeds=torch.rand((9,512)).cuda()
        inputs=torch.concat([inputs,inputs],dim=0) #假装有一份sup
        x = self.extract_feat(inputs)
        return self.decode_head.forward([inputs, x, classifier_embeds],[],1)
